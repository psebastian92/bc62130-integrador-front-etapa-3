...
export const get = async (url) => {
  try {
    ...
    if (!respuesta.ok) {
      throw new Error(
        `Algo paso: ${respuesta.status}, ${respuesta.statusText}`
      );
    }
...
export const post = async (url, dato) => {
  try {
    ...
    if (!respuesta.ok) {
      throw new Error(
        `Algo pasó: ${respuesta.status}, ${respuesta.statusText}`
      );
    }
    const productoGuardado = await respuesta.json(); // Añadidos paréntesis aquí
...
export const put = async (url, id, dato) => {
  try {
    ...
    if (!respuesta.ok) {
      throw new Error(
        `Algo ocurrió, ${respuesta.status} ${respuesta.statusText}`
      );
    }
